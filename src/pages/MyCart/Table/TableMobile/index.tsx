      ...
      {cart.map((item) => (
        <div key={`${item.snack}-${item.id}`} className='order-item'>
          ...
          <div>
            ...
            <div>
              <div>
              <button type='button' onClick={() => snackCartDecrement(item)} aria-label='Decrementar quantidade'>
                  <img src={minusImg} alt='Remover quantidade' />
                </button>
                <span>{`${item.quantity}`.padStart(2, '0')}</span>
                <button type='button' onClick={() => snackCartIncrement(item)} aria-label='Incrementar quantidade'>
                  <img src={plusImg} alt='Adicionar quantidade' />
                </button>
              </div>
              <button type='button' onClick={() => removeSnackFromCart(item)} aria-label='Remover do carrinho'>
                <FaTrashAlt />
              </button>
            </div>
            <h5>
